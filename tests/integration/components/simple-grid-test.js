...
test('it should adapt to the columns count', function(assert) {
  ...
  return run.next(() => {
    assert.equal(this.$('.grid-item').width(), 90, 'width of elem');
    assert.equal(this.$('.grid-item').height(), 300, 'height of elem');

    this.set('columns', 2);

    return run.next(() => {
      assert.equal(this.$('.grid-item').width(), 140, 'width of elem');
      assert.equal(this.$('.grid-item').height(), 300, 'height of elem');
    });
  });
});
